...
def train_one_epoch(model, loader, optimizer, scheduler, device):
    ...
    for features, species_targets, family_targets in loader:
        features, species_targets, family_targets = (
            features.to(device),
            species_targets.to(device),
            family_targets.to(device),
        )
...
if __name__ == "__main__":
    ...
    train_ds = SafariSupervisedDataset(
        "./data/sa_fari_train_ext.json", "./data/embeddings"
    )
    val_ds = SafariSupervisedDataset(
        "./data/sa_fari_test_ext.json", "./data/embeddings"
    )
    ...
    model = SafariMultiTaskClassifier(base_student, 384, num_species, num_families).to(
        device
    )
