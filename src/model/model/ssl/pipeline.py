...
class SSLPipeline:
    ...
    def extract_features(
        self, input_dir, batch_size=16, num_workers=4, overwrite=False
    ):
        ...
        model = torch.hub.load("facebookresearch/dinov2", self.teacher_type).to(
            self.device
        )
        model.eval()
    ...
    def train_ssl(
        self, video_input_dir, epochs=30, batch_size=32, lr=1e-3, start_epoch=None
    ):
        ...
        print(
            f"Training SSL Student: {self.student_type} from Teacher: {self.teacher_type}"
        )
        ...
        for epoch in range(start_epoch + 1 if start_epoch is not None else 0, epochs):
            ...
            torch.save(
                model.state_dict(), f"student_epoch{epoch+1}_{self.student_type}.pth"
            )
