...
class SafariSupervisedDataset(Dataset):
    def __init__(self, json_path, embeddings_dir):
        ...
        for c in categories:
            ...
            fam_name = (
                str(fam)
                if fam and not (isinstance(fam, float) and torch.tensor(fam).isnan())
                else "Unknown"
            )
        ...
        for ann in self.data["annotations"]:
            if os.path.exists(
                os.path.join(embeddings_dir, f"sa_fari_{ann['video_id']:06d}.pt")
            ):
    ...
    def __getitem__(self, idx):
        ...
        emb_path = os.path.join(
            self.embeddings_dir, f"sa_fari_{ann['video_id']:06d}.pt"
        )
