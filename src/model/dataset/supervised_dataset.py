...
class SafariSupervisedDataset(Dataset):
    def __init__(self, json_path, frames_root, num_frames=8, transform=None):
        ...
        unique_families = sorted(list(set([str(c.get("Family", "Unknown")) for c in categories])))
        self.family_map = {name: i for i, name in enumerate(unique_families)}
        self.sp_to_fam = {c["id"]: self.family_map[str(c.get("Family", "Unknown"))] for c in categories}
    ...
    def __getitem__(self, idx):
        ...
        indices = torch.linspace(0, len(sample["frame_names"]) - 1, self.num_frames).long()
